...
const MAX_TRANSIT_MESSAGE_SIZE: usize = 1024 * 1024;
#[cfg(not(target_family = "wasm"))]
use std::{
...
pub(super) trait TransitTransportRx: AsyncRead + std::any::Any + Unpin + Send {
    ...
    async fn read_transit_message(&mut self) -> Result<Vec<u8>, std::io::Error> {
        ...
        if length > MAX_TRANSIT_MESSAGE_SIZE {
            return Err(std::io::Error::new(
                std::io::ErrorKind::InvalidData,
                "transit record exceeds 1 MiB limit",
            ));
        }
...
#[cfg(test)]
mod tests {
    use super::TransitTransportRx;

    #[test]
    fn oversized_transit_record_is_rejected_before_body_read() {
        let header = (2 * 1024 * 1024_u32).to_be_bytes();
        let mut stream = futures::io::Cursor::new(header);
        let error = futures::executor::block_on(stream.read_transit_message()).unwrap_err();
